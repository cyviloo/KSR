...
public class FeatureCalculator {
	...
	public Sample getOtherAsSample(Observation toAnalyse) {
		...
		for(Map.Entry<String, Double> s : base.getFeatures().entrySet()) {
			String maxString = null;
			double maxSimilarity = 0;
			for(Map.Entry<String, Double> os : observation.getFeatures().entrySet()) {
				String osString = os.getKey();
				double similarity = measurer.getSimilarity(s.getKey(), osString);
